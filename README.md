
<h1 align="center"> 🇪🇸 Perfil en Español </h1>

---
# Pedro Martínez Cernadas 👨‍💻
[**HAGA CLICK si desea ver mi curriculum**](https://github.com/xDit0x/xDiT0x/blob/main/Curriculum%20Vitae.pdf)

[![Email](https://img.shields.io/badge/Email-Contact%20Me-red)](mailto:pedromarcer02@gmail.com)
[![Status](https://img.shields.io/badge/Status-Open%20to%20Opportunities-brightgreen)]()
[![IT Specialist - Java](https://img.shields.io/badge/Credly-IT_Specialist_Java-FF6B35?logo=credly&logoColor=white)](https://www.credly.com/badges/cc280784-3150-47b8-8aaa-8042a0086b25/public_url)

## 📋 Resumen Profesional

Estudiante de segundo curso de **Ingeniería Informática** y piloto certificado de drones. Destaco por mi responsabilidad, capacidad de aprendizaje rápido y habilidades avanzadas en lengua inglesa. Enfocado en el desarrollo de software y aplicaciones web innovadoras.

---

## 🎓 Formación Académica

### **Grado en Ingeniería Informática** 
`🏫 Universidad Europea (UE)` | `📅 Cursando actualmente`
- Especial interés en desarrollo de software e inteligencia artificial

### **Bachillerato Científico**

---

## 📜 Certificaciones

| Certificación | Entidad | Estado |
|---------------|---------|---------|
| 🚁 **Piloto STS-ES-01** | AESA | ✅ Obtenido |
| 🚁 **Piloto STS-ES-02** | AESA | ✅ Obtenido |
| 🇬🇧 **Cambridge English Advanced (C1)** | Cambridge | ✅ **182/210** (Grade A) |
| 🗣️ **Certificado de Valenciano (C1)** | JQCV | ✅ Obtenido |
| ☕ **IT Specialist - Java** | Certiport/Credly | ✅ [Ver credencial](https://www.credly.com/badges/cc280784-3150-47b8-8aaa-8042a0086b25/public_url) |

---

## 💻 Stack Tecnológico

### **Lenguajes de Programación**
[![IT Specialist - Java](https://img.shields.io/badge/Java-IT_Specialist-orange?style=flat-square&logo=java&logoColor=white)](https://www.credly.com/badges/your-badge-id)
![C++](https://img.shields.io/badge/C++-Intermediate-blue?style=flat-square)
![JavaScript](https://img.shields.io/badge/JavaScript-Básico-yellow?style=flat-square)

### **Desarrollo Web**
![HTML5](https://img.shields.io/badge/HTML5-Expert-E34F26?style=flat-square&logo=html5)
![CSS3](https://img.shields.io/badge/CSS3-Expert-1572B6?style=flat-square&logo=css3)
![React](https://img.shields.io/badge/React-Learning-61DAFB?style=flat-square&logo=react)

---

## 🛠️ Habilidades Técnicas

### **Desarrollo & Programación**
- Programación orientada a objetos
- Desarrollo de aplicaciones web
- Fundamentos de ingeniería de software
- Resolución de problemas algorítmicos
- **Certificación IT Specialist en Java**

### **Operación de Drones**
- Operación bajo normativa STS-ES
- Conocimientos de aeronáutica
- Seguridad en operaciones aéreas

### **Idiomas**
- **Español**: Nativo
- **Inglés**: C1 (Advanced - Grade A)
- **Valenciano**: C1

---

## 🚀 Proyecto Actual
<img src="https://github.com/xDit0x/xDiT0x/blob/main/LogosProIng.png?raw=true
" width="300" alt="DRBOT logo">
### **🏥 Aplicación Médica con IA**
**Descripción:** Desarrollo de una aplicación médica orientada a mejorar la eficiencia del sistema de consultas mediante **Inteligencia Artificial**.

**Tecnologías:** 
`Java` `React` `AI/ML` `Web Development`

**Objetivo:** Optimizar la gestión de consultas médicas y mejorar la experiencia del paciente.

---

## 🌟 Habilidades Personales

| Habilidad | Nivel |
|-----------|-------|
| Trabajo en equipo | ⭐⭐⭐⭐⭐ |
| Aprendizaje rápido | ⭐⭐⭐⭐⭐ |
| Responsabilidad | ⭐⭐⭐⭐⭐ |
| Comunicación efectiva | ⭐⭐⭐⭐ |
| Pensamiento analítico | ⭐⭐⭐⭐ |

---

## 📈 Estadísticas Académicas

```python
# Rendimiento Académico
nota_bachillerato = 8.26
nivel_ingles = "C1 (182/210)"
certificaciones = 5  # Incluye IT Specialist - Java
certificacion_java = "IT Specialist"
```

---

## 🎯 Objetivos Profesionales

- 🔍 **Busco oportunidades** en desarrollo de software y aplicaciones web
- 📚 **Continuar formación** en inteligencia artificial y machine learning para la continuación en programas de la nueva era
- 🌐 **Participar en proyectos** innovadores que generen impacto social y trascendencia histórica **(MAYOR OBJETIVO 🎯)**
- 🚀 **Contribuir al desarrollo** de soluciones tecnológicas avanzadas bajo un equipo enfocado en la búsqueda de la excelencia en la codificación
- ☕ **Aplicar mis conocimientos certificados** de Java en proyectos reales, y de gran relevancia en el sector tecnológico

---

## 📞 Contacto

- 📧 **Email**: [pedromarcer02@gmail.com](mailto:pedromarcer02@gmail.com)
- 🏫 **Universidad**: Universidad Europea
- 🏆 **Credly**: [Perfil de certificaciones](https://www.credly.com/users/pedro-martinez-cernadas)
  

---

<h1 align="center"> 🇬🇧 English Profile </h1>

---

# Pedro Martínez Cernadas 👨‍💻
[**CLICK HERE to view my CV**](https://github.com/xDit0x/xDiT0x/blob/main/Curriculum%20Vitae.pdf)

[![Email](https://img.shields.io/badge/Email-Contact%20Me-red)](mailto:pedromarcer02@gmail.com)
[![Status](https://img.shields.io/badge/Status-Open%20to%20Opportunities-brightgreen)]()
[![IT Specialist - Java](https://img.shields.io/badge/Credly-IT_Specialist_Java-FF6B35?logo=credly&logoColor=white)](https://www.credly.com/badges/cc280784-3150-47b8-8aaa-8042a0086b25/public_url)

## 📋 Professional Summary

Second-year **Computer Engineering** student and certified drone pilot. I stand out for my responsibility, fast learning ability, and advanced English language skills. Focused on developing innovative software and web applications.

---

## 🎓 Education

### **Bachelor's Degree in Computer Engineering**
`🏫 European University (UE)` | `📅 Currently enrolled`
- Special interest in software development and artificial intelligence

### **Scientific Baccalaureate**

---

## 📜 Certifications

| Certification | Entity | Status |
|---------------|---------|---------|
| 🚁 **STS-ES-01 Pilot** | AESA (Spanish Aviation Safety Agency) | ✅ Obtained |
| 🚁 **STS-ES-02 Pilot** | AESA | ✅ Obtained |
| 🇬🇧 **Cambridge English Advanced (C1)** | Cambridge | ✅ **182/210** (Grade A) |
| 🗣️ **Valencian Language Certificate (C1)** | JQCV | ✅ Obtained |
| ☕ **IT Specialist - Java** | Certiport/Credly | ✅ [View credential](https://www.credly.com/badges/cc280784-3150-47b8-8aaa-8042a0086b25/public_url) |

---

## 💻 Tech Stack

### **Programming Languages**
[![IT Specialist - Java](https://img.shields.io/badge/Java-IT_Specialist-orange?style=flat-square&logo=java&logoColor=white)](https://www.credly.com/badges/your-badge-id)
![C++](https://img.shields.io/badge/C++-Intermediate-blue?style=flat-square)
![JavaScript](https://img.shields.io/badge/JavaScript-Basics-yellow?style=flat-square)

### **Web Development**
![HTML5](https://img.shields.io/badge/HTML5-Expert-E34F26?style=flat-square&logo=html5)
![CSS3](https://img.shields.io/badge/CSS3-Expert-1572B6?style=flat-square&logo=css3)
![React](https://img.shields.io/badge/React-Learning-61DAFB?style=flat-square&logo=react)

---

## 🛠️ Technical Skills

### **Development & Programming**
- Object-Oriented Programming
- Web Application Development
- Software Engineering Fundamentals
- Algorithmic Problem Solving
- **IT Specialist Certification in Java**

### **Drone Operations**
- Operation under STS-ES regulations
- Aeronautical knowledge
- Aerial operation safety

### **Languages**
- **Spanish**: Native
- **English**: C1 (Advanced - Grade A)
- **Valencian**: C1

---

## 🚀 Current Project
<img src="https://github.com/xDit0x/xDiT0x/blob/main/LogosProIng.png?raw=true
" width="300" alt="DRBOT logo">
### **🏥 Medical Application with AI**
**Description:** Development of a medical application aimed at improving the efficiency of the consultation system through **Artificial Intelligence**.

**Technologies:**
`Java` `React` `AI/ML` `Web Development`

**Goal:** Optimize medical appointment management and improve the patient experience.

---

## 🌟 Personal Skills

| Skill | Level |
|-----------|-------|
| Teamwork | ⭐⭐⭐⭐⭐ |
| Fast Learner | ⭐⭐⭐⭐⭐ |
| Responsibility | ⭐⭐⭐⭐⭐ |
| Effective Communication | ⭐⭐⭐⭐ |
| Analytical Thinking | ⭐⭐⭐⭐ |

---

## 📈 Academic Statistics

```python
# Academic Performance
baccalaureate_grade = 8.26
english_level = "C1 (182/210)"
certifications_count = 5  # Includes IT Specialist - Java
java_certification = "IT Specialist"
```

---

## 🎯 Professional Goals

- 🔍 **Seeking opportunities** in software and web application development
- 📚 **Continue training** in artificial intelligence and machine learning for the new era's programs
- 🌐 **Participate in innovative projects** that generate social impact and historical significance **(MAIN GOAL 🎯)**
- 🚀 **Contribute to the development** of advanced technological solutions within a team focused on achieving coding excellence
- ☕ **Apply my certified Java knowledge** to real-world projects of significant relevance in the tech sector

---

## 📞 Contact

- 📧 **Email**: [pedromarcer02@gmail.com](mailto:pedromarcer02@gmail.com)
- 🏫 **University**: European University
- 🏆 **Credly**: [Certification Profile](https://www.credly.com/users/pedro-martinez-cernadas)


<div align="center">
  <sub>• • •</sub>
</div>  
  
